...
async function buildMenu(env: Env) {
  ...
  const categories = cats.map((c) => ({
    ...
    selectionType: c.SelectionType === 'multi' ? 'multi' as const : c.SelectionType === 'optional' ? 'optional' as const : 'single' as const,
    maxSelections: c.MaxSelections != null ? Number(c.MaxSelections) : null,
    icon:          s(c.Icon),

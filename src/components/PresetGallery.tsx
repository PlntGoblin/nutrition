...
import { presets, selectionCount, selections, setFormat } from "../lib/store";
...
function applyPreset(preset: PresetBowl, onSelect: () => void): void {
...
export function PresetGallery({ onSelect }: Props): JSX.Element | null {
  ...
  return (
    <section class="nc-presets" aria-labelledby="nc-presets-eyebrow">
      <header class="nc-presets__head">
        ...
        <p class="nc-presets__lede">Tap one to pre-fill your build — customize from there.</p>
      </header>
      <div class="nc-presets__grid" role="list">
        {list.map(preset => (
          <button
            ...
            role="listitem"
            onClick={() => applyPreset(preset, onSelect)}
          >
            <div class="nc-preset-card__photo">
              <img src={preset.image} alt="" loading="lazy" width={400} height={300} />
            </div>
              ...
              {preset.tags.length > 0 && (
                <ul class="nc-preset-card__tags">
                  {preset.tags.slice(0, 3).map(tag => (
                    <li key={tag} class="nc-preset-card__tag">{tag}</li>
                  ))}
                </ul>
              )}
            </div>
